...
*/
void gamma_coef(double *coef, double *py, double *pdta, int *dim) {
    int i, j, k, count = 0;
    double itmp, tmp, v;
...
// tp <- rnorm(p, 1)
// y <- as.double(rgamma(n, exp(X %*% tp)/5, 5))
// dyn.load("test.so")
// system.time(b <- .C("gamma_coef", coef = double(p), y, X, dim(X), DUP = FALSE)$coef)
// system.time(print(cc <- glm(y ~ 0 + X, family = Gamma(link = "log"))$coef, digit = 22))
// print(b, digit = 22)
// print(tp, digits = 22)
