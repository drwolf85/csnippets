...
/**
 * @brief Inverse Gaussian (linear) Regression  (with log-link function)
 * 
...
*/
void invGauss_coef(double *coef, double *py, double *pdta, int *dim) {
    int i, j, k, count = 0;
    double itmp, tmp, v;
    ...
    if (q && r && y && w && dta && jcb) {
        ...
        do {
            ...
            for (j = 0; j < dim[0]; j++) {
                ...
                w[j] = y[j];
                y[j] = py[j] - exp(w[j]);
                w[j] = exp(1.5 * w[j]); /* Sqrt of Variance function */
                for (i = 0; i < dim[1]; i++)
