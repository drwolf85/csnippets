...
/**
 * @brief Negatiive Binomial (linear) Regression  (with log-link function)
 * 
...
*/
void nbinom_coef(double *coef, double *py, double *pdta, int *dim) {
    int i, j, k, count = 0;
    double itmp, tmp, v;
...
// tp <- rnorm(p, 1)
// y <- as.double(rnbinom(n, 5, 1/(1+5/exp(X %*% tp))))
// dyn.load("test.so")
// system.time(b <- .C("nbinom_coef", coef = double(p), y, X, dim(X), DUP = FALSE)$coef)
// system.time(print(cc <- glm(y ~ 0 + X, family = quasi(link = "log", variance = "mu+mu^2"))$coef, digit = 22))
// print(b, digit = 22)
// print(tp, digits = 22)
// sqrt(mean((y- exp(X%*%tp))^2)); sqrt(mean((y-exp(X%*%cc))^2)); sqrt(mean((y- exp(X%*%b))^2))
