...
} model_t; /* Piecewise LINear fit that can be discontinuous. */
...
typedef struct leaf_model {
    ...
    double cm_l; /* Parameters for the left side */
    double cm_r; /* Parameters for the right side */
	...
	if (K < *k_max && n >= *n_fit && *n_leaf >= 2 && *n_fit > *n_leaf * 2) {
		...
		if (T->mod.best_model == CON) {
			...
			for (i = 0; i < n; i++) {
				...
				raw_pred[ell]  = T->mod.cm_l * (double) (x <  T->mod.best_pivot);
				raw_pred[ell] += T->mod.cm_r * (double) (x >= T->mod.best_pivot);
				/* Residual update */
 ...
 */
tree_t * cart(double *y, double *X, int *dimX, int *Kmax, int *nFit, int *nLeaf) {
...
/**
 * @brief Function to compute the CART predictions
 * 
 * @param T Pointer to a root node of a CART model
 * @param x Pointer to a vector of features
 * @param p Pointer to the number of variables (i.e., the length of `x`)
 * @return double
 */
double cart_predict(tree_t const *T, double *x, int const *p) {
	double pred = 0.0;
	...
	if (T && x && p) {
		...
		while(Tpt) {
			...
			if (tmpx < Tpt->mod.best_pivot) {
				pred = clamp(pred + Tpt->mod.cm_l, -3.0 * B, B * 3.0);
				Tpt = Tpt->l;
			}
			else {
				pred = clamp(pred + Tpt->mod.cm_r, -3.0 * B, B * 3.0);
				Tpt = Tpt->r;
			}
		}
		pred += T->C;
...
/* Memory management functions for the CART model */
...
int main() {
	...
	double pred, obs;
	size_t i, j;
	tree_t *myCART = cart(y, x, dimX, &Kmax, &nFit, &nLeaf);
	if (myCART) {
		printf("# Testing CART model\n");
		printf("---\nc(");
		for (i = 0; i < nobs - 1; i++) {
			obs = y[i];
			for (j = 0; j < nvar; j++) {
			...
			pred = cart_predict(myCART, xtest, &dimX[1]);
			/* printf("\tObserved: %g\n", obs);
			printf("\tPredicted: %g\n", pred);
			printf("---\n");
			printf("\tResiduals: %g\n", obs - pred); */
