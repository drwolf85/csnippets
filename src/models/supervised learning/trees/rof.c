...
tree_t * plof_fit(double const *Y, int *dimY, double const *X, int *dimX, int n_trees, int *Kmax, int const *nFit, int const *nLeaf, bool const centering, bool const clamping) {
	int i, j;
	tree_t *forest = NULL;
	datum *data;
...
int main() {
	...
	size_t i, j;
		...
		printf("# ---\n");
	srand(time(NULL));
	tree_t *myPLOF = plof_fit(y, dimY, x, dimX, NTREES, &Kmax, &nFit, &nLeaf, MYCENTER, MYCLAMP);
	/*if (myPLOF) {
		printf("err <- c(");
		for (i = 0; i < nobs - 1; i++) {
			obs = y[i];
			...
			pred = plof_predict(myPLOF, xtest, &dimX[1], MYCENTER, MYCLAMP_Y, MYCLAMP_X);
			/* printf("\tObserved: %g\n", obs);
			printf("\tPredicted: %g\n", pred);
			printf("---\n");
			printf("\tResiduals: %g\n", obs - pred); */ /*
