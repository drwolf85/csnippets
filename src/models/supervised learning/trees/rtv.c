...
static void build_tree(node *T, datum *dta, uint64_t n, uint8_t lev, uint64_t *p, uint8_t *max_depth, uint8_t *mnsn) {
  ...
  if (n >= (((uint64_t) *mnsn) << 1ULL) && lev < *max_depth) {
    ...
    vec *spread = (vec *) calloc(*p, sizeof(vec));
    if (spread) {
      /* Pick a variable by checking the performance of each predictor */
      for (j = 0; j < *p; j++) {
        whv = spread[j].i = j;
	...
	for (i = nmn + 1; i < nmx; i++) {
      ...
      qsort(spread, *p, sizeof(vec), cmp_vec);
      whv = T->var2split = spread->i;
      T->split = spread->aux;
    }
    free(spread);
...
int main() {
  ...
  printf("Training of a CART model terminated (with loop over all input variables).\n");
  if (mytree) {
    ptx[0] = 0.54321;
    ptx[1] = -0.54321;
    ptx[2] = 0.0;
