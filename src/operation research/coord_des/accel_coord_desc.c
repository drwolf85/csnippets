...
/**
 * Accelerated coordinate descent (without strong convexity)
 *
 * @param param the parameters to be optimized
 * @param len the length of the parameter vector
 ...
 * @param objf a routine that computes the objective function
 */
void accel_coord_desc(double *param, int *len, int *n_iter, void *info,
          double (*objf)(double *, int *, void *)) {
    int t, i, np = *len;
    ...
    if (grd && desc && x && y && z) {    
        ...
        for (t = 0; t < *n_iter * np; t++) {
            i = grd[0].i;
            x[i] = y[i] - LEARNING_RATE * grd[0].v * desc[i];
            z[i] = y[i] - LEARNING_RATE * grd[0].v * desc[i] / (theta * (double) np);
            if (newf >= oldf) break;
...
int main() {
    ...
    printf("\t x = %g, y = %g\n", unk[0], unk[1]);
    printf("\t fun(x, y) = %g\n", function(unk, &p, 0));
    accel_coord_desc(unk, &p, &maxit, 0, function);
    ...
    printf("\n");
    printf("\t fun(x_vec) = %g\n", function2(unk, &p, 0));
    accel_coord_desc(unk, &p, &maxit, 0, function2);
    printf("Optimized results:\n");
    printf("\t ");
    i = 0;
    printf("x[%d] = %g", i, unk[i]);
